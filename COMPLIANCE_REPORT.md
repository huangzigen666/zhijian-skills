...
| SBOM 仅覆盖 Node skill | 范围限定 | Python 侧（playwright/`wcx`）未生成 SBOM |
...
**建议后续（非阻塞）**：① 在干净 macOS 账号实跑 theme-studio live doctor 以闭合最后两项 live 缺口；② 视需要为 Python 侧补充 SBOM；③ 定期重跑 `verify-upstream` 与 `gen-node-sbom.mjs` 以跟踪依赖漂移。
