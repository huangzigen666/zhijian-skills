...
本 skill 的联网抓取**完全委托**仓库外的 `web-clipper` skill，且通过 `bash` 调用其脚本。该调用模式若处理不当会发生**命令注入（RCE）**，须严格遵守以下规则：

1. **`WEB_CLIPPER_ROOT` 必须可信且格式受限**：只接受绝对路径，且只能包含 `[A-Za-z0-9._/-]` 字符；拒绝任何含引号、`$`、`` ` ``、`;`、空格等 shell 元字符的值。永远不要从网页内容、会话历史或不可信输入拼接该路径。
2. **`--url` 必须严格校验后再代入**：仅允许以 `https://` 开头、且不含 `"` `'` `$` `` ` `` `;` `|` `&` `(` `)` `<` `>` 换行 空格 等字符的 URL。任何从抓取的网页中提取的子链接、或对话中提供的"官网 URL"，在写入命令行前都必须先过此校验；不通过则中止，绝不原样拼接。
3. **用 argv 数组方式调用，而非字符串拼接**：优先通过一个小包装函数 `exec "$WEB_CLIPPER_ROOT/scripts/run_web_clipper.sh" "$@"` 以**位置参数**传递 `--url`，使 URL 作为单一参数传递，绝不把 `--url "…"` 拼进一个 shell 字符串后整体 `bash -c`。即便如此，第 2 条的 URL 校验仍是必需的第一道防线。
4. **企业名称 → 仓库目录必须净化**：目录名只保留 `[A-Za-z0-9._-]`，剔除 `/`、`..` 及其他路径元字符，禁止用原始企业名称直接拼路径（防止目录穿越）。
5. **在专用工作目录中运行**，不要在当前工作目录（CWD）下直接产出，避免污染或越界写盘。

> 详细调用安全示例见 `references/web-clipper-usage.md` 的「安全约束」小节。
...
| 官网URL | 否 | 有则抓取官网，无则跳过（**须按「安全约束」严格校验后再使用**） |
| 本地资料路径 | 否 | 客户提供的资料文件夹路径 |
| 输出路径 | 否 | 默认当前目录下创建 `{企业名}-企业分身/`（`企业名` 须按「安全约束」净化，建议在专用工作目录产出） |
